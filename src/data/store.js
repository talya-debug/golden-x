...
import { masterPriceList, demoQuotes, demoProjects, demoMilestones, demoProjectTasks, demoPurchases, demoWorkLogs, demoSubcontractors, findPriceItem, defaultMilestones } from './mockData'

function load(key, fallback) {
  try {
    const d = localStorage.getItem(`gx_${key}`)
    return d ? JSON.parse(d) : fallback
  } catch { return fallback }
}
function save(key, data) { localStorage.setItem(`gx_${key}`, JSON.stringify(data)) }
